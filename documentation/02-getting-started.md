...
- **.NET 10** or later. The packages target `net10.0`. To run the samples from this repository, you need the .NET SDK that its `global.json` names, or a newer one of the same major version.
...
- `StoreHealth` is the object that watches the [store](01-concepts.md#store) (here, Redis) across every limiter that shares it: it holds the **[store timeout](01-concepts.md#store-timeout)** and the **[circuit breaker](01-concepts.md#circuit-breaker)**. `StoreHealthOptions` here uses its defaults (20 ms timeout; the breaker opens when at least 5 calls happen within the last 10 seconds, and at least half of them failed). Create one `StoreHealth` per store connection, and pass it to every limiter that uses that connection; [The circuit breaker](01-concepts.md#the-circuit-breaker) explains why.
...
Every lease this library hands out is also counted in a metric, so you do not have to read logs to know how often the fallback answered instead of Redis. The web sample exports it with OpenTelemetry (a standard, vendor-neutral way to collect and send metrics, logs and traces):
