...
Why a console program instead of BenchmarkDotNet: BenchmarkDotNet's memory diagnoser reports bytes *allocated* per operation, which is not the same as bytes *kept alive* per partition — a trial run showed about 2,800 B allocated against about 1,000 B actually retained. On top of that, xUnit runs test classes in parallel, and its own runner allocates on the same process heap, which would pollute a heap-size measurement taken during a test run. A single console process per scenario, with `GC.GetTotalMemory(true)` read before and after building all 10,000 partitions, avoids both problems.
