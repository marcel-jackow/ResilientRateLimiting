...
**Limits on how long warm state is kept.** Keeping state for every client costs memory. A [partition](#partition) that has seen no traffic for a while is normally removed by `PartitionedRateLimiter`'s cleanup of idle limiters (this is how .NET's `PartitionedRateLimiter` behaves — third-party behaviour, not something this library controls). The library tells the cleanup to keep a partition's limiter while it holds warm state, but only within two limits:
...
1. **A base value.**
   ...
   - If a healthy store rejected without a value, there is **no** Retry-After at all. The library has nothing honest to estimate from. **Measured** (see [measurements.md](measurements.md#redisratelimiting-and-the-retry-after-value)): `RedisRateLimiting` 1.2.1's sliding window, fixed window and token bucket limiters give no value under the standard `MetadataName.RetryAfter` name. (The fixed window and token bucket limiters give a value under their own metadata name, `RATELIMIT_RETRYAFTER`, which this library does not read; the sliding window limiter gives none.) So with these limiters, a rejection by a healthy Redis has no Retry-After.
...
2. **Added time, capped.** A random spread is always added, so that many rejected clients do not all return at the same moment and cause a new spike: normally 10-20% of the base value, at least 1 second on the low side and at least 3 seconds on the high side. While degraded, the spread is wider — 20-40% of the base value, at least 2 seconds on the low side and at least 6 seconds on the high side — and a longer wait (up to the base value again) is added too, because the store is already in trouble. Both additions together never exceed `MaxAddedRetryDelay` (60 seconds by default, a **starting point**). Set it to zero to add nothing.
...
A wrapper around another object that adds behaviour without changing how callers use it. `ResilientRateLimiting` is a decorator around a store-backed [rate limiter](#rate-limiter): you call it exactly like the limiter it wraps.
